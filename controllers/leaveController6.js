import Leave6 from '../models/Leave6.js';
...
const getLeave = async (req, res) => {
    try {
        ...
        if (role === "admin") {
            leaves = await Leave6.find({ employeeId: id });
        } else {
            const employee = await Employee.findOne({ userId: id });
            leaves = await Leave6.find({ employeeId: employee._id });
        }
...
const getLeaves = async (req, res) => {
    try {
        const leaves = await Leave6.find().populate({
            path: "employeeId",
            populate: [
...
const getLeaveDetail = async (req, res) => {
    try {
        ...
        const leave = await Leave6.findById(id).populate({
            path: "employeeId",
            populate: [
                { path: 'department', select: 'dep_name' },
...
const updateLeave = async (req, res) => {
    try {
        ...
        const leave = await Leave6.findByIdAndUpdate(
            id,
            { 
                status,
