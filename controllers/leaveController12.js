import Leave12 from '../models/Leave12.js';
...
const getLeave = async (req, res) => {
    try {
        ...
        if (role === "admin") {
            leaves = await Leave12.find({ employeeId: id });
        } else {
            const employee = await Employee.findOne({ userId: id });
            leaves = await Leave12.find({ employeeId: employee._id });
        }
...
const getLeaves = async (req, res) => {
    try {
        const leaves = await Leave12.find().populate({
            path: "employeeId",
            populate: [
...
const getLeaveDetail = async (req, res) => {
    try {
        ...
        const leave = await Leave12.findById(id).populate({
            path: "employeeId",
            populate: [
                { path: 'department', select: 'dep_name' },
...
const updateLeave = async (req, res) => {
    try {
        ...
        const leave = await Leave12.findByIdAndUpdate(
            id,
            { 
                status,
