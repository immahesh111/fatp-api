import Leave8 from '../models/Leave8.js';
...
const getLeave = async (req, res) => {
    try {
        ...
        if (role === "admin") {
            leaves = await Leave8.find({ employeeId: id });
        } else {
            const employee = await Employee.findOne({ userId: id });
            leaves = await Leave8.find({ employeeId: employee._id });
        }
...
const getLeaves = async (req, res) => {
    try {
        const leaves = await Leave8.find().populate({
            path: "employeeId",
            populate: [
...
const getLeaveDetail = async (req, res) => {
    try {
        ...
        const leave = await Leave8.findById(id).populate({
            path: "employeeId",
            populate: [
                { path: 'department', select: 'dep_name' },
...
const updateLeave = async (req, res) => {
    try {
        ...
        const leave = await Leave8.findByIdAndUpdate(
            id,
            { 
                status,
