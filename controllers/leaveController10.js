import Leave10 from '../models/Leave10.js';
...
const getLeave = async (req, res) => {
    try {
        ...
        if (role === "admin") {
            leaves = await Leave10.find({ employeeId: id });
        } else {
            const employee = await Employee.findOne({ userId: id });
            leaves = await Leave10.find({ employeeId: employee._id });
        }
...
const getLeaves = async (req, res) => {
    try {
        const leaves = await Leave10.find().populate({
            path: "employeeId",
            populate: [
...
const getLeaveDetail = async (req, res) => {
    try {
        ...
        const leave = await Leave10.findById(id).populate({
            path: "employeeId",
            populate: [
                { path: 'department', select: 'dep_name' },
...
const updateLeave = async (req, res) => {
    try {
        ...
        const leave = await Leave10.findByIdAndUpdate(
            id,
            { 
                status,
