import Leave7 from '../models/Leave7.js';
...
const getLeave = async (req, res) => {
    try {
        ...
        if (role === "admin") {
            leaves = await Leave7.find({ employeeId: id });
        } else {
            const employee = await Employee.findOne({ userId: id });
            leaves = await Leave7.find({ employeeId: employee._id });
        }
...
const getLeaves = async (req, res) => {
    try {
        const leaves = await Leave7.find().populate({
            path: "employeeId",
            populate: [
...
const getLeaveDetail = async (req, res) => {
    try {
        ...
        const leave = await Leave7.findById(id).populate({
            path: "employeeId",
            populate: [
                { path: 'department', select: 'dep_name' },
...
const updateLeave = async (req, res) => {
    try {
        ...
        const leave = await Leave7.findByIdAndUpdate(
            id,
            { 
                status,
